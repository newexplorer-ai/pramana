...
# ---------------------------------------------------------------- curation
#
# The admin-curated priority order, newest revision. Position is meaningful:
# providers cap allowed_domains (100 on OpenAI), so a pool is searched in
# priority order and anything past the cap is only reached on a later pass.
# Each list is exactly at the cap, so each region searches in a single call.
#
# This drives `priority` on every boot. It deliberately does NOT drive
# `enabled` — that stays admin-managed, so a redeploy never silently undoes
# a toggle made in the admin panel.
